...
class ModelRouter:
    ...
    def __model_router(self, model_type: str = None, base_url: str = None) -> dict:
        ...
        chat_gpt_model_list = ["gpt-", "o", "claude", "gemini"]
        llm_model_to_use = (
            model_type if model_type is not None else os.environ.get("MODEL")
        )

        is_vllm_model = "/" in llm_model_to_use and ":" not in llm_model_to_use

        is_gpt_model = (
            (
                len([x for x in chat_gpt_model_list if llm_model_to_use.startswith(x)])
                > 0
        ...
        return {
            ...
            "provider": (
                "openai" if is_gpt_model else "vllm" if is_vllm_model else "ollama"
            ),
        }
    ...
    def get_model(
        ...
        standard_params = {
            ...
            "temperature": (
                temperature if not model_to_use["model"].startswith("o4") else 1.0
            ),
        }
        ...
        if model_to_use["provider"] == "vllm":
            standard_params["openai_api_key"] = os.environ.get(
                "OPENAI_API_KEY", "ollama"
            )
            model_to_use["provider"] = "openai"
